...
const DecodeRegistrations = () => {
  ...
  const fetchRegistrations = async (page = 1) => {
    ...
    try {
      ...
      // Using the same service; backend can filter by type if supported.
      const response = await decodeRegistrationService.getRegistrations(
        pagination,
        filters
      );
      if (response.success) {
  ...
  const exportToCSV = async (startDate, endDate) => {
    ...
    try {
      ...
      }decode-registration/download?${params.toString()}`;
      ...
      if (!response.ok) {
        // If it's a 404 (no data found), get the error message
        if (response.status === 404) {
          const errorData = await response.json();
          alert(
            errorData.message || "No data found for the selected date range"
          );
          return;
        }
      ...
      // Check if the response is CSV (backend might be returning JSON error even on 200)
      ...
      // If we get here, it's a successful CSV response
      const blob = await response.blob();
      const downloadUrl = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = downloadUrl;
      a.download = "decode_registrations.csv";
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex items-center space-x-2 text-gray-600">
              ...
              <span className="text-sm">
                Found: {registrations.length} results
              </span>
            </div>
          </div>
        </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                          <p>
                            ...
                            {selectedRegistration.levelName || "N/A"}
                          </p>
                          <p>
                            <span className="font-medium">Date & Time:</span>{" "}
                            {selectedRegistration.city.split(" | ")[3] || "N/A"}
                          </p>
                          <p>
                            <span className="font-medium">Course Detail:</span>{" "}
                            {selectedRegistration.city.split(" | ")[2] || "N/A"}
                          </p>
                        </div>
                      </div>
...
export default DecodeRegistrations;
